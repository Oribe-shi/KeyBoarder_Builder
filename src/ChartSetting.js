...
function ChartSetup(SetupBackGroundColor) {
    ...
    Time = Math.floor(ChartMusic[Music].duration * 1000) + LoadDelayTime + Date.now();
    ...
    //譜面一時停止可能に
    document.addEventListener("keydown", ChartPauseKeyEvent);
    //ウィンドウからフォーカスが外れたら一時停止
    window.addEventListener("blur", ChartPause);
}

let MusicBeforeTime = 0;
let MusicProgress = 0;
let PauseWaitTime = 0;
/**
 * 経過時間計測関数
 */
function PastTime() {
    if (ChartMusic[Music].currentTime > 0) {
        MusicProgress = ChartMusic[Music].currentTime * 1000 + MusicBeforeTime;
    } else {
        MusicBeforeTime = Date.now() - PauseWaitTime;
        MusicProgress = Date.now() - PauseWaitTime;
    }
...
function PlayMusic(MusicName) {
    ...
    mySetTimeout(function() {
        MusicName.play();
    }, LoadDelayTime + OtherData.MusicGap + MusicErrorTime + Judge_Time.MISSEARLY + Judge_Time.BADEARLY + Judge_Time.EARLY + 0.5 * Judge_Time.PERFECT);
}
...
/**
 * 一時停止・途中再開可能のanime関数
 * @param {Object} 実行するanime関数のオブジェクト
 */
function ChartAnimation(animeObject) {

    animeObject.complete = function() {
        TimeLines.Anime.TimeList.splice(TimeLines.Anime.Instance.indexOf(SetAnimationList), 1);
        TimeLines.Anime.Instance.splice(TimeLines.Anime.Instance.indexOf(SetAnimationList), 1);
    }

    const SetAnimationList = anime(animeObject);
    TimeLines.Anime.Instance.push(SetAnimationList);

    //[開始時間, 時間(ポーズしたときに代入)]
    TimeLines.Anime.TimeList.push([ChartMusic[Music].currentTime * 1000, 0]);

}

/**
 * 一時停止・途中再開可能のsetTimeout関数
 * @param {Function} 実行する任意の関数
...
function mySetTimeout(Function, Time) {
    ...
    TimeLines.setTimeout.ID.push(TimerID);
    //[開始時間, 待機時間, 待機後に実行する関数]
    TimeLines.setTimeout.TimeList.push([MusicProgress, Time, Function]);
...
function ClearTimeoutData(TimerID) {
    ...
    let index = TimeLines.setTimeout.ID.indexOf(TimerID);
    if (index != -1) {
        TimeLines.setTimeout.ID.splice(index, 1);
        TimeLines.setTimeout.TimeList.splice(index, 1);
    }
 ...
 * ポーズ画面から開始して動くまでtrue
 */
let PauseSetTimeoutFrag = false;
/**
 * ポーズ画面から再開するまでの時間(ms)
 */
const ResumeDurationTime = 3000;
let PauseStartTime = 0;
let PauseWindoe_Animation;
let ResumeWindoe_Animation;
/**
 * 譜面一時停止関数
 */
function ChartPause() {
    ...
    clearInterval(PauseSetTimeout);

    isPaused = true;
    window.removeEventListener("blur", ChartPause);

    //ポーズ画面を表示
    PauseMusicText.innerHTML = "一時停止したため、" + document.querySelector("#MusicSelectTemp" + SelectMusicListNumber).getAttribute("alt") + ".exeを再起動する必要があります。";
    anime.remove("#PauseWindow");
    anime.remove("#ResumeWindow");
    PauseWindow.style.opacity = 1;
    ...
    //曲を一時停止
    ChartMusic[Music].pause();

    //一時停止再開途中に一時停止した場合は、そのときの記録は残さないようにする
    if (PauseSetTimeoutFrag) return;
    PauseSetTimeoutFrag = true;

    MusicStopTime = ChartMusic[Music].currentTime * 1000;
    MusicResumeTime = MusicStopTime - ResumeDurationTime; //開始までの待機時間(3000ms)

    //anime.jsの一時停止
    TimeLines.Anime.Instance.forEach(function(animeInstance, index) {

        //一時停止したときの時間を代入
        TimeLines.Anime.TimeList[index][1] = ChartMusic[Music].currentTime * 1000;
        animeInstance.pause();

    });

    //setTimeoutの一時停止
    TimeLines.setTimeout.ID.forEach(function(ID, index) {

        clearTimeout(ID);

        //それぞれの待機時間から、開始してから一時停止するまでの時間を引く
        TimeLines.setTimeout.TimeList[index][1] -= (MusicProgress - TimeLines.setTimeout.TimeList[index][0]);
...
function ChartResume() {
    ...
    ChartMusic[Music].currentTime = (MusicStopTime - ResumeDurationTime) / 1000;
    ...
    anime({
        targets: "#ResumeSeekBar",
        width: ["0%", "100%"],
        duration: 3000,
        easing: "linear",
        update: function(anim) {
            ResumeMusicText.innerHTML = document.querySelector("#MusicSelectTemp" + SelectMusicListNumber).getAttribute("alt") + ".exe";
            ResumePercent.innerHTML = Math.round(anim.progress) + "%";
            ResumeSeconds.innerHTML = "あと" + (3 - Math.floor(anim.progress / (100/3))) + "秒";
        }
    });

    function ResumeCount() {
        //譜面の進行を再開
        if (MusicStopTime == 0) PauseWaitTime += Date.now() - PauseStartTime;
        ChartMode = setInterval(PastTime, 0);

        //anime.jsの再開
        TimeLines.Anime.Instance.forEach(function(animeInstance, index) {

            console.log(index, animeInstance.progress);
            if (MusicStopTime == 0) animeInstance.seek(TimeLines.Anime.TimeList[index][1] - TimeLines.Anime.TimeList[index][0]);
            animeInstance.play();
        ...
        TimeLines.setTimeout.ID.forEach(function(ID, index) {
            ...
            let ResumeTime = MusicProgress;

            let TimerID = setTimeout(function() {
                TimeLines.setTimeout.TimeList[TimeLines.setTimeout.ID.indexOf(TimerID)][2]();
                // setTimeoutが終了したらその情報を削除
                ClearTimeoutData(TimerID);
            }, TimeLines.setTimeout.TimeList[index][1]);

            TimeLines.setTimeout.ID[index] = TimerID;
            TimeLines.setTimeout.TimeList[index][0] = ResumeTime;

        });

        PauseSetTimeoutFrag = false;
    }

    let ResumeFlag = true;
    function ResumeFlagFunction() {

        console.log(ChartMusic[Music].currentTime)

        if (MusicResumeTime < 0 && ResumeFlag) {

            if (MusicStopTime == 0) {
                //まだ曲が開始されていない場合
                console.log("まだ")
                clearInterval(PauseSetTimeout);
                PauseSetTimeout = setTimeout(ResumeCount, ResumeDurationTime);
                return;
            } else {
                //巻き戻したときに曲がまだ開始されていない場合
                clearInterval(PauseSetTimeout);
                PauseSetTimeout = setTimeout(function() {

                    ResumeFlag = false;
                    ChartMusic[Music].play();
                    PauseSetTimeout = setInterval(ResumeFlagFunction, 0);

                }, ResumeDurationTime - MusicStopTime);
            }

        } else {

            //曲を再開
            if (ChartMusic[Music].paused) ChartMusic[Music].play();

        }
        if (ChartMusic[Music].currentTime * 1000 >= MusicStopTime) {
...
function ChartInit(StopMusic) {
    ...
    //一時停止機能を終了
    document.removeEventListener("keydown", ChartPauseKeyEvent);
    window.removeEventListener("blur", ChartPause);
    ...
    mySetTimeout(Delay, LoadDelayTime);
}
    ...
    static Note() {
        return {
            ...
            Eva(EditNumber, Authenticity, Delay) {
                ...
                if (Authenticity) {
                    const Judge = ms => new Promise(func => mySetTimeout(func, ms));
                            ...
                            function NoteKeyEvent(Result, JudgeTime) {
                                ...
                                function Keydown(NoteTAP) {
                                    ...
                                    if (Notetype) {
                                        ...
                                        if (NoteTAP.key === Words[Notetype] && keyCount > BeforekeyCount && Keydown_Event && !PauseSetTimeoutFrag) {
                                    ...
                                    //ワードフリーノーツの場合
                                        ...
                                        if (NoteTAP.key.charCodeAt(0) >= 97 && NoteTAP.key.charCodeAt(0) <= 122 && keyCount > BeforekeyCount && Keydown_Event && !isSameNote && !PauseSetTimeoutFrag) {
                ...
                if (Authenticity) {
                    ...
                    async function Judgement() {
                        ...
                        if (!AutoMode) {
                            ...
                            const LongNoteConditionTimeout = mySetTimeout(function() {
                            ...
                            function LongNoteKeyEvent(Result, JudgeTime) {
                                ...
                                function Keydown(LongNoteTAP) {
                                    ...
                                    if (Notetype) {
                                        ...
                                        if (LongNoteTAP.key === Words[Notetype] && keyCount > BeforekeyCount && Keydown_Event && !PauseSetTimeoutFrag) {
                                            ...
                                            if (NoteInfoList.SameNumberList[NoteInfoList.WithInRangeNumberList.indexOf(EditNumber)] == 1) {
                                                ...
                                                if (Result == 5) {
                                                    ...
                                                    document.addEventListener("keyup", function Keyup(LongNoteTAP) {
                                                        ...
                                                        if (LongNoteTAP.key === Words[Notetype] && Keyup_Event && !PauseSetTimeoutFrag) {
                                    ...
                                    //ワードフリーノーツの場合
                                        ...
                                        if (LongNoteTAP.key.charCodeAt(0) >= 97 && LongNoteTAP.key.charCodeAt(0) <= 122 && keyCount > BeforekeyCount && Keydown_Event && !isSameNote && !PauseSetTimeoutFrag) {
                                            ...
                                            if (NoteInfoList.SameNumberList[NoteInfoList.WithInRangeNumberList.indexOf(EditNumber)] == 1) {
                                                ...
                                                if (Result == 5) {
                                                    ...
                                                    document.addEventListener("keyup", function Keyup() {
                                                        ...
                                                        if (Object.keys(keys).length == 0 && Keyup_Event && !PauseSetTimeoutFrag) {
                    ...
                    async function Judgement() {
                        ...
                        function DragNoteKeyEvent() {
                            ...
                            if (Notetype) {
                                ...
                                if (keys[Words[Notetype]] != null && !DragNoteFrag && !PauseSetTimeoutFrag) {
                            ...
                            //ワードフリーノーツの場合
                                ...
                                if (Object.keys(keys).length && !DragNoteFrag && !PauseSetTimeoutFrag) {
